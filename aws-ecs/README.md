# Terraform for Piiano Vault AWS ECS

This module deploys Piiano Vault on a given AWS region.

## Solution Architecture

Vault is deployed as a single container on ECS cluster. The service is deployed in the ECS VPC and is configured for private access only from the `pvault-vpc`.
Internally, Vault communicates with a Postgres RDS that resides in the database subnet.

![piiano-vault](piiano-vault-aws-apprunner.png "terraform-piiano-vault-ecs")
...
| Instance-role | IAM Instance role for the ECS instance        |                                                             |
| VPC endpoint  | Private VPC Endpoint for App Runner           |                                                             |
| ECS cluster   | Managed ECS cluster with Fargate capacity     | Optional - Can be replaced with existing ECS parameters     |
| ECS service   | Managed ECS service deployment of Piiano Vault|                                                             |
...
3. To reuse your existing ECS cluster, disable the creation by setting `create_ecs_cluster` to `false` and configure the parameter for `ecs_cluster_id`. You can find your ECS ID in the AWS console.
...
| <a name="input_create_ecs_cluster"></a> [create\_vpc](#input\_create\_ecs\_cluster) | n/a | `bool` | `true` | no |
...
| <a name="input_ecs_cluster_id"></a> [vpc\_id](#input\_ecs\_cluster\_id) | The existing ECS\_ID | `string` | `""` | no |
